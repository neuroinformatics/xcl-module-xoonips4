...
class Xoonips_PolicyItemFieldAction extends Xoonips_ActionBase {
	...
	protected function doRegistersave(&$request, &$response) {
		...
		$viewData['url'] = XOOPS_URL.'/modules/'.$this->dirname.'/admin/index.php?action=PolicyItemField';
		if ($mode == 1) $viewData['redirect_msg'] = _AM_XOONIPS_POLICY_ITEMTYPE_DETAIL_RELEASE_MSG_SUCCESS;
		else $viewData['redirect_msg'] = _AM_XOONIPS_POLICY_ITEMTYPE_DETAIL_REGIST_MSG_SUCCESS;
		$response->setViewData($viewData);
		$response->setForward('registersave_success');
		return true;
	}
	...
	protected function doEdit(&$request, &$response) {
		...
		if ($detail_info['preselect'] == 1) $disedi = true;
	...
	protected function doEditsave(&$request, &$response) {
		...
		$viewData['url'] = XOOPS_URL.'/modules/'.$this->dirname.'/admin/index.php?action=PolicyItemField';
		if ($mode == 1) $viewData['redirect_msg'] = _AM_XOONIPS_POLICY_ITEMTYPE_DETAIL_RELEASE_MSG_SUCCESS;
		else $viewData['redirect_msg'] = _AM_XOONIPS_POLICY_ITEMTYPE_DETAIL_MODIFY_MSG_SUCCESS;
		$response->setViewData($viewData);
		$response->setForward('editsave_success');
		return true;
	}
	...
	private function doCopyItemfield( $itemfieldid, $isCopy = false, &$insertId ) {
		$map = array();
		if ($isCopy) {
			$detailBean = Xoonips_BeanFactory::getBean('ItemFieldDetailBean',$this->dirname,$this->trustDirname);
			if (!$detailBean->copyById($itemfieldid, $map)) return false;

		}
		else
		{
			$detailBean = Xoonips_BeanFactory::getBean('ItemFieldDetailBean',$this->dirname,$this->trustDirname);
			if (!$detailBean->copyById($itemfieldid, $map, true)) return false;
