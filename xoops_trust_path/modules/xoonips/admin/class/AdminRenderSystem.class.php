...
class Xoonips_AdminRenderSystem extends Legacy_AdminRenderSystem {
	...
	public static function getOverrideFileInfo($file, $prefix = null, $isSpDirName = false) {
		...
		if ($isModule && file_exists($path = sprintf('%s/modules/%s/%s', XOOPS_THEME_PATH, $theme, $dirName, $file))) {
			...
			$ret['path'] = $path;
		} else if ($isModule && file_exists($path = sprintf('%s/themes/%s/modules/%s/%s', XOOPS_TRUST_PATH, $theme, $trustDirName, $file))) {
			$ret['path'] = $path;
			$ret['dirname'] = $trustDirName;
		} else if (file_exists($path = sprintf('%s/%s/%s', XOOPS_THEME_PATH, $theme, $file))) {
			$ret['url'] = sprintf('%s/%s/%s', XOOPS_THEME_URL, $theme, $file);
			$ret['path'] = $path;
			$ret['dirname'] = null;
		} else if (file_exists($path = sprintf('%s/themes/%s/%s', XOOPS_TRUST_PATH, $theme, $file))) {
			$ret['path'] = $path;
			$ret['dirname'] = null;
		} else if ($isModule && file_exists($path = sprintf('%s/%s/admin/templates/%s', XOOPS_MODULE_PATH, $dirName, $file))) {
			$ret['url'] = sprintf('%s/%s/admin/templates/%s', XOOPS_MODULE_URL, $dirName, $file);
			$ret['path'] = $path;
			$ret['theme'] = null;
		} else if ($isModule && file_exists($path = sprintf('%s/modules/%s/admin/templates/%s', XOOPS_TRUST_PATH, $trustDirName, $file))) {
			$ret['path'] = $path;
			$ret['theme'] = null;
			$ret['dirname'] = $trustDirName;
		} else if (file_exists($path = self::$_mFallbackPath . '/' .$file)) {
