...
class XmlIndexExport
{
    ...
    {
        $title = $top_arr['title'];
        if (0 != strcmp($index_path_arr[0], $title)) {
            return [];
        }
        ...
        foreach ($index_path_arr as $path) {
            ...
            foreach ($child as $child_index) {
                ...
                if (0 == strcmp($ret['title'], $path)) {
                    $parent_index_id = $ret['index_id'];
                    $result = $ret;
                }
            }
            if (-1 == $parent_index_id) {
                return [];
            }
        }
    ...
    private function walk_private_index($uid, &$index_path_arr)
    {
        ...
        $item_ids = [];
        $children = [];
        if (false == empty($priindex['index_id'])) {
    ...
    private function walk_public_index($uid, &$index_path_arr)
    {
        ...
        $item_ids = [];
        $children = [];
        if (false == empty($pubindex['index_id'])) {
    ...
    private function walk_group_index($uid, &$index_path_arr)
    {
        ...
        foreach ($grp_top_arrs as $grp_top_arr) {
            ...
            $item_ids = [];
            $children = [];
            if (false == empty($grpindex['index_id'])) {
    ...
    private function getPathArray($indexpath)
    {
        ...
        $index_path_arr = array_merge(array_diff($explode, ['']));
    ...
    private function mkdom(&$arr)
    {
        ...
        if (false == empty($arr['current'])) {
            $cuuent = $index_data->appendChild($dom->createElementNS($url, 'I:'.'current'));
            $node_arr = ['title', 'index_id', 'description', 'open_level', 'weight'];
    ...
    public function getDom($indexpath, $uid, $user)
    {
        ...
        if (0 == strcmp($index_path_arr[0], 'Public')) {
            // Public Index
        ...
        } elseif (0 == strcmp($index_path_arr[0], $user)) {
            // Private Index
