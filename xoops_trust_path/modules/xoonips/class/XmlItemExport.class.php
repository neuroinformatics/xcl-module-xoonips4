...
class XmlItemExport
{
    ...
    private function dispatch_tbl(&$ids, &$item_field_detail_element, &$grp_dom)
    {
        ...
        if (false !== $rc) {
            ...
            // other proc call
            if (true == method_exists($this, $table_name)) {
        ...
        foreach ($item_field_detail as $value) {
            if (empty($ret[$value['group_id']])) {
                $ret[$value['group_id']] = [
                    'xml' => $value['xml'],
                    'field_group' => $value,
                    'item_field_detail_id' => [$value['item_field_detail_id']],
                ];
            } else {
