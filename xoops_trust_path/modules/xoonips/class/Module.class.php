...
class Xoonips_Module extends Legacy_ModuleAdapter {
		...
		if (!is_array($adminMenu)) {
			...
			if (!empty($fname)) {
				if (file_exists($path = XOOPS_ROOT_PATH . '/modules/' . $dirname . '/' . $fname)) {
					include $path;
					$adminMenu = $adminmenu;
				} else if (file_exists($path = XOOPS_TRUST_PATH . '/modules/' . $trustDirname . '/' . $fname)) {
					include $path;
					$adminMenu = $adminmenu;
				}
			}
	...
	public function getPreferenceEditUrl() {
		if ($this->_mPreferenceEditUrl === null) {
			...
			} else if (is_array($this->mXoopsModule->getInfo('config')) && count($this->mXoopsModule->getInfo('config')) > 0) {
				$root =& XCube_Root::getSingleton();
